...
// Around line 2500 - REPLACE toggleTydexSidebar function

/**
 * Toggle Tydex sidebar visibility - FIXED WITH EXPLICIT STYLES
 */
function toggleTydexSidebar() {
    ...
    isTydexSidebarCollapsed = !isTydexSidebarCollapsed;
    
    if (isTydexSidebarCollapsed) {
        // ✅ COLLAPSED STATE: Hide sidebar completely
        sidebar.style.width = '0';
        sidebar.style.minWidth = '0';
        sidebar.style.maxWidth = '0';
        ...
        if (toggleBtn) {
            toggleBtn.style.cssText = `
                ...
                cursor: pointer !important;
            `;
            toggleBtn.title = 'Show Tydex Files';
        }
        
        // Show expand icon (arrow pointing left - expand from right)
        if (toggleIcon) {
            toggleIcon.className = 'fas fa-chevron-left';
            toggleIcon.style.fontSize = '20px';
            toggleIcon.style.color = '#fff';
        ...
        if (toggleBtn) {
            toggleBtn.style.cssText = `
                ...
                justify-content: center !important;
            `;
            toggleBtn.title = 'Hide Tydex Files';
        }
        
        // Show collapse icon (arrow pointing right - collapse to right)
        if (toggleIcon) {
            toggleIcon.className = 'fas fa-chevron-right';
            toggleIcon.style.fontSize = '13px';
            toggleIcon.style.color = '#6b3a9b';
...
// ✅ Add global variable at top of file
let isTydexSidebarCollapsed = false;

// Stop auto-refresh when page is hidden (save resources)
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        stopLogsAutoRefresh();
        console.log('⏸️ Page hidden - logs refresh paused');
    } else {
        startLogsAutoRefresh();
        fetchAndDisplayLogs();
        console.log('▶️ Page visible - logs refresh resumed');
    }
});
...
function downloadLogs() {
    ...
    const content = logsContent.innerText || logsContent.textContent;
    if (!content) {
    ...
    const filename = `${projectName}_${protocol}_${timestamp}.log`;
 ...
 * ✅ FIXED: Only auto-scroll if user is already at bottom
 */
async function fetchAndDisplayLogs() {
    ...
    if (!projectName || !protocol) {
        console.warn('⚠️ No project/protocol found for logs');
        return;
    }
    ...
    try {
        ...
        if (data.success && data.content) {
            // ✅ Display logs
            logsContent.innerHTML = `<pre style="margin: 0; white-space: pre-wrap; word-break: break-word; font-family: 'Courier New', monospace; font-size: 12px; color: #000;">${escapeHtml(data.content)}</pre>`;
            
            // ✅ Update status
            if (logsStatus) {
                logsStatus.textContent = 'Live';
                logsStatus.className = 'running';
            }
            ...
            console.log(`✅ Logs loaded: ${data.content.length} characters (auto-scroll: ${isAtBottom})`);
