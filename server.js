...
app.post('/api/run-batch-file', async (req, res) => {
    try {
        ...
        // ✅ Fetch test data for Tydex generation (CDTire only)
        let testDataMap = {};
        if (normalizedProtocol === 'CDTire') {
            const query = `
                SELECT 
                    number_of_runs, job, old_job, p, l,
                    template_tydex, tydex_name
                FROM cdtire_data 
            ...
            // Create map: runNumber -> rowData
            result.rows.forEach(row => {
                testDataMap[row.number_of_runs] = row;
            });
        }
        ...
        batchProcess.stdout.on('data', async (data) => {
            const output = data.toString();
            logger.info(output);
            
            // ✅ Detect which run we're processing (cd /d command changes folder)
            const folderMatch = output.match(/cd \/d ".*?([^\\]+)\\([^\\]+)"$/m);
            if (folderMatch) {
                const p = folderMatch[1];
                const l = folderMatch[2];
                
                // Find run number from P_L folder
                for (const [runNum, rowData] of Object.entries(testDataMap)) {
                    if (`${rowData.p}_${rowData.l}` === `${p}_${l}`) {
                        currentRun = runNum;
                        logger.info(`📂 Switched to Run ${runNum} (${p}_${l})`);
                        break;
                    }
                }
            }
            
            // ✅ Detect job execution
            if (output.includes('call abaqus job=')) {
                currentStep = 'ABAQUS_JOB';
                if (currentRun) {
                    await updateRunStatus(normalizedProtocol, currentRun, 'Running: Abaqus Job');
                    logger.info(`⚙️ Run ${currentRun}: Executing Abaqus job`);
                }
            }
            
            // ✅ Detect Python script execution
            if (output.includes('call abaqus python')) {
                currentStep = 'PYTHON_SCRIPT';
                if (currentRun) {
                    await updateRunStatus(normalizedProtocol, currentRun, 'Running: Python Post-Processing');
                    logger.info(`🐍 Run ${currentRun}: Executing Python script`);
                }
            }
            
            // ✅ Detect job completion (Abaqus exit messages)
            if (output.includes('Abaqus JOB') && output.includes('COMPLETED')) {
                if (currentRun && currentStep === 'ABAQUS_JOB') {
                    await updateRunStatus(normalizedProtocol, currentRun, 'Completed ✓');
                    logger.success(`✅ Run ${currentRun}: Job completed`);
                    
                    // ✅ Generate Tydex file after job completion (CDTire only)
                    if (normalizedProtocol === 'CDTire') {
                        const rowData = testDataMap[currentRun];
                        if (rowData && rowData.template_tydex && rowData.template_tydex !== '-') {
                            logger.info(`📄 Run ${currentRun}: Generating Tydex file...`);
                            await generateTydexForRun(projectName, normalizedProtocol, rowData, logger);
                        }
                    }
                }
            }
            ...
            if (output.includes('Batch execution completed')) {
        ...
        batchProcess.stderr.on('data', (data) => {
            ...
            if (currentRun) {
                logger.error(`❌ Error in Run ${currentRun}: ${error}`);
            }
        });
        
        batchProcess.on('close', (code) => {
            if (code === 0) {
                logger.success(`✅ Batch execution finished (exit code: ${code})`);
            } else {
                logger.error(`❌ Batch execution failed (exit code: ${code})`);
            }
        });
        
        // Register process for tracking
        registerChildProcess(batchProcess, { 
            projectName, 
        ...
        res.json({
            success: true,
            message: 'Batch execution started. Check logs for live progress.'
        });
...
// Around line 3600 - ADD/REPLACE this endpoint
...
// Around line 3180-3400

// Around line 3180-3400

/**
 * Generate batch file for automated test execution
 * ✅ FIXED: CDTire includes Fortran + Python commands in correct sequence
 * Other protocols: job → oldjob → tydex (unchanged)
 */
// Around line 3180-3400
...
async function generateBatchFile(projectName, protocol) {
    try {
        ...
        // ✅ Fetch test data with Fortran/Python columns for CDTire
        let query;
        if (protocol === 'CDTire') {
            query = `
                SELECT 
                    number_of_runs, job, old_job, p, l,
                    fortran_file, python_script
                FROM ${tableName} 
                ORDER BY number_of_runs ASC
            `;
        } else {
            query = `SELECT number_of_runs, job, old_job, p, l FROM ${tableName} ORDER BY number_of_runs ASC`;
        }
...
for (const test of testData) {
    ...
    const job = test.job ? test.job.trim() : '';
    const oldJob = test.old_job ? test.old_job.trim() : '';
    const fortran = test.fortran_file ? test.fortran_file.trim() : '';
    const python = test.python_script ? test.python_script.trim() : '';
    const p = test.p || '';
    const l = test.l || '';
    
    // Skip if missing critical data
    if (!job || !p || !l) {
        console.log(`⚠️  Skipping run ${runNumber}: missing job/p/l`);
        skippedCount++;
        continue;
    }
    ...
    // ✅ ALWAYS print REM + path for EVERY test case
    if (commandCount > 0) {
        batchContent += `\n`; // Blank line before each new test
    }
    
    batchContent += `REM Run ${runNumber}\n`;
    batchContent += `cd /d "${folderPath}"\n`;
    
    // Update currentFolder tracker
    currentFolder = folderName;
    
    // ============================================
    // ✅ STEP 1: Build Abaqus Command (FILENAME ONLY - NO PATH)
    // ============================================
    const jobFileName = job.replace(/\.inp$/i, '');
    
    // ✅ CRITICAL FIX: Use ONLY filename (no folder path) since we cd'd into directory
    let abaqusCmd = `call abaqus job=${jobFileName}`;
    
    // ✅ Add oldjob if specified (dependency from SAME folder)
    if (oldJob && oldJob !== '-') {
        const oldJobFileName = oldJob.replace(/\.inp$/i, '');
        abaqusCmd += ` oldjob=${oldJobFileName}`;
    }
    
    // ✅ Add input parameter with FILENAME ONLY (not "P2_L1\file.inp")
    abaqusCmd += ` input=${job}`;
    
    // ============================================
    // ✅ CDTire WORKFLOW: Add Fortran user subroutine
    // ============================================
    if (protocol === 'CDTire' && fortran && fortran !== '-') {
        // ✅ Fortran files are stored in PROJECT ROOT after generation
        // Example: projects\1_CDTire\8_freeroll_p2_l1_0_60.f
        const fortranFileName = fortran.endsWith('.f') ? fortran : `${fortran}.f`;
        const fortranPath = path.join('..', fortranFileName); // Relative to P2_L1 folder
        abaqusCmd += ` user="${fortranPath}"`;
    }
    
    abaqusCmd += ` cpus=${ABAQUS_CPU_COUNT} interactive\n`;
    ...
    if (protocol === 'CDTire' && python && python !== '-') {
        ...
        // Format from database: "od_growth.py 1_axi_p1.odb" or "deflection.py 2_rev_P2_L1_0.odb speed2"
        const pythonParts = python.split(/\s+/);
        const scriptName = pythonParts[0]; // e.g., "od_growth.py"
        const scriptArgs = pythonParts.slice(1).join(' '); // e.g., "1_axi_p1.odb speed2"
        
        // ✅ Python script path: ..\od_growth.py (relative to P2_L1 folder)
